...
use elexeum_node_network_protocol::{PeerId, UnifiedReputationChange};
...
use elexeum_node_subsystem_types::{
	jaeger,
...
fn extract_metrics(registry: &prometheus::Registry) -> HashMap<&'static str, u64> {
	...
	let activated = extract("elexeum_parachain_activated_heads_total");
	let deactivated = extract("elexeum_parachain_deactivated_heads_total");
	let relayed = extract("elexeum_parachain_messages_relayed_total");
