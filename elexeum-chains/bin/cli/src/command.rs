...
pub use elexeum_performance_test::PerfCheckError;
...
impl SubstrateCli for Cli {
	...
	fn support_url() -> String {
		"https://github.com/elexeum/elexeum-node/issues/new".into()
	}
	...
	fn load_spec(&self, id: &str) -> std::result::Result<Box<dyn sc_service::ChainSpec>, String> {
		...
		Ok(match id {
			"elexeum" => Box::new(service::chain_spec::elexeum_config()?),
			#[cfg(feature = "elexeum-native")]
			"dev" | "elexeum-dev" => Box::new(service::chain_spec::elexeum_development_config()?),
			#[cfg(feature = "elexeum-native")]
			"elexeum-local" => Box::new(service::chain_spec::elexeum_local_testnet_config()?),
			#[cfg(feature = "elexeum-native")]
			"elexeum-staging" => Box::new(service::chain_spec::elexeum_staging_testnet_config()?),
			#[cfg(not(feature = "elexeum-native"))]
			name if name.starts_with("elexeum-") && !name.ends_with(".json") =>
				Err(format!("`{}` only supported with `elexeum-native` feature enabled.", name))?,
			...
			path => {
				...
				let chain_spec = Box::new(service::ElexeumChainSpec::from_json_file(path.clone())?)
	...
	fn native_runtime_version(spec: &Box<dyn service::ChainSpec>) -> &'static RuntimeVersion {
		...
		#[cfg(feature = "elexeum-native")]
		{
			return &service::elexeum_runtime::VERSION
		}

		#[cfg(not(feature = "elexeum-native"))]
		panic!("No runtime feature (elexeum, cardamom) is enabled")
	}
}
...
const DEV_ONLY_ERROR_PATTERN: &'static str =
	"can only use subcommand with --chain [elexeum-dev, cardamom-dev, -dev], got ";
...
/// Parses elexeum specific CLI arguments and run the service.
pub fn run() -> Result<()> {
	let cli: Cli = Cli::from_args();

	match &cli.subcommand {
		None => run_node_inner(cli, service::RealOverseerGen, elexeum_node_metrics::logger_hook()),
		...
		Some(Subcommand::ExportBlocks(cmd)) => {
			...
			Ok(runner.async_run(|mut config| {
				let (client, _, _, task_manager) =
					service::new_chain_ops(&mut config, None).map_err(Error::ElexeumService)?;
		...
		Some(Subcommand::PvfPrepareWorker(cmd)) => {
			...
			{
				elexeum_node_core_pvf::prepare_worker_entrypoint(&cmd.socket_path);
				Ok(())
			}
		},
		Some(Subcommand::PvfExecuteWorker(cmd)) => {
			...
			#[cfg(not(target_os = "android"))]
			{
				elexeum_node_core_pvf::execute_worker_entrypoint(&cmd.socket_path);
				Ok(())
			}
		},
		Some(Subcommand::Benchmark(cmd)) => {
			...
			// else we assume it is elexeum.
			#[cfg(feature = "elexeum-native")]
			{
				return Ok(runner.sync_run(|config| {
					cmd.run::<service::elexeum_runtime::Block, service::ElexeumExecutorDispatch>(
						config,
					)
					.map_err(|e| Error::SubstrateCli(e))
				})?)
