...
use elexeum_node_subsystem_util::metrics::{self, prometheus};
use elexeum_overseer::{AllMessages, Handle};
use elexeum_primitives::v1::{
	Block as ElexeumBlock, BlockNumber, Hash, Header as ElexeumHeader,
};
use elexeum_subsystem::messages::{
...
const MAX_FINALITY_LAG: elexeum_primitives::v1::BlockNumber = 500;
		...
		let metrics = MetricsInner {
			approval_checking_finality_lag: prometheus::register(
				prometheus::Gauge::with_opts(
					prometheus::Opts::new(
						"elexeum_parachain_approval_checking_finality_lag",
			...
			disputes_finality_lag: prometheus::register(
				prometheus::Gauge::with_opts(
					prometheus::Opts::new(
						"elexeum_parachain_disputes_finality_lag",
...
/// into account.
enum IsDisputesAwareWithOverseer<B: sc_client_api::Backend<ElexeumBlock>> {
...
where
	B: sc_client_api::Backend<ElexeumBlock>,
...
pub struct SelectRelayChain<B: sc_client_api::Backend<ElexeumBlock>> {
	longest_chain: sc_consensus::LongestChain<B, ElexeumBlock>,
...
where
	B: sc_client_api::Backend<ElexeumBlock> + 'static,
{
	...
	pub fn as_longest_chain(&self) -> &sc_consensus::LongestChain<B, ElexeumBlock> {
		&self.longest_chain
	}
}

#[async_trait::async_trait]
impl<B> SelectChain<ElexeumBlock> for SelectRelayChain<B>
where
	B: sc_client_api::Backend<ElexeumBlock> + 'static,
{
	...
	async fn best_chain(&self) -> Result<ElexeumHeader, ConsensusError> {
		match self.selection {
...
where
	B: HeaderProviderProvider<ElexeumBlock>,
	OH: OverseerHandleT,
{
	...
	fn block_header(&self, hash: Hash) -> Result<ElexeumHeader, ConsensusError> {
...
where
	B: HeaderProviderProvider<ElexeumBlock> + Send + Sync,
	OH: OverseerHandleT,
{
	fn clone(&self) -> Self {
