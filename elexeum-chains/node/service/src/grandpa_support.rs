...
//! Elexeum-specific GRANDPA integration utilities.  
...
mod tests {
	...
	use elexeum_test_client::{
		ClientBlockImportExt, DefaultTestClientBuilderExt, InitElexeumBlockBuilder,
	...
	fn grandpa_pause_voting_rule_works() {
		...
		let mut push_blocks = {
			...
			move |n| {
				for _ in 0..n {
					let block = client.init_elexeum_block_builder().build().unwrap().block;
