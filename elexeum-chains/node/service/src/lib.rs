...
use {
	...
	elexeum_node_core_approval_voting::Config as ApprovalVotingConfig,
	elexeum_node_core_av_store::Config as AvailabilityConfig,
	elexeum_node_core_av_store::Error as AvailabilityError,
	elexeum_node_core_candidate_validation::Config as CandidateValidationConfig,
	elexeum_node_core_chain_selection::{
		self as chain_selection_subsystem, Config as ChainSelectionConfig,
	},
	elexeum_node_core_dispute_coordinator::Config as DisputeCoordinatorConfig,
	elexeum_overseer::BlockInfo,
...
pub use {
	...
	elexeum_overseer::{Handle, Overseer, OverseerConnector, OverseerHandle},
	elexeum_primitives::v2::ParachainHost,
...
pub use elexeum_client::CardamomExecutorDispatch;

#[cfg(feature = "elexeum-native")]
pub use elexeum_client::ElexeumExecutorDispatch;

pub use chain_spec::{CardamomChainSpec, ElexeumChainSpec};
...
#[cfg(feature = "full-node")]
pub use elexeum_client::{
	AbstractClient, Client, ClientHandle, ExecuteWithClient, FullBackend, FullClient,
	RuntimeApiCollection,
};
pub use elexeum_primitives::v1::{Block, BlockId, CollatorPair, Hash, Id as ParaId};
pub use service::{
...
#[cfg(feature = "cardamom-native")]
pub use cardamom_runtime;
#[cfg(feature = "elexeum-native")]
pub use elexeum_runtime;
...
pub enum Error {
	...
	Overseer(#[from] elexeum_overseer::SubsystemError),
	...
	#[error(transparent)]
	Jaeger(#[from] elexeum_subsystem::jaeger::JaegerError),
	...
	#[cfg(feature = "full-node")]
	#[error("Expected at least one of elexeum, cardamom runtime feature")]
	NoRuntime,
}

/// Can be called for a `Configuration` to identify which network the configuration targets.
pub trait IdentifyVariant {
	/// Returns if this is a configuration for the `Cardamom` network.
	fn is_cardamom(&self) -> bool;
...
impl IdentifyVariant for Box<dyn ChainSpec> {
	fn is_cardamom(&self) -> bool {
		self.id().starts_with("cardamom") || self.id().starts_with("cdm")
	}
	fn is_dev(&self) -> bool {
	...
	ChainSelection: 'static + SelectChain<Block>,
{
	...
	let rpc_extensions_builder = {
		...
		move |deny_unsafe,
		      subscription_executor: elexeum_rpc::SubscriptionTaskExecutor|
		      -> Result<elexeum_rpc::RpcExtension, service::Error> {
			let deps = elexeum_rpc::FullDeps {
				client: client.clone(),
				pool: transaction_pool.clone(),
				select_chain: select_chain.clone(),
				chain_spec: chain_spec.cloned_box(),
				deny_unsafe,
				babe: elexeum_rpc::BabeDeps {
					babe_config: babe_config.clone(),
					shared_epoch_changes: shared_epoch_changes.clone(),
					keystore: keystore.clone(),
				},
				grandpa: elexeum_rpc::GrandpaDeps {
			...
			elexeum_rpc::create_full(deps).map_err(Into::into)
		}
	};
...
{
	use elexeum_node_network_protocol::request_response::IncomingRequest;
	...
	let disputes_enabled = true;

	let pvf_checker_enabled = false;

	let select_chain = if requires_overseer_for_chain_sel {
		let metrics =
			elexeum_node_subsystem_util::metrics::Metrics::register(prometheus_registry.as_ref())?;
	...
	// Note: GrandPa is pushed before the Elexeum-specific protocols. This doesn't change
	...
	{
		use elexeum_network_bridge::{peer_sets_info, IsAuthority};
...

		{
			...
			task_manager.spawn_essential_handle().spawn_blocking(
				...
				Box::pin(async move {
					...
					let forward = elexeum_overseer::forward_events(overseer_client, handle);
	...
	if role.is_authority() {
		...
		let babe_config = babe::BabeParams {
			...
			create_inherent_data_providers: move |parent, ()| {
				...
				async move {
					let parachain = elexeum_node_core_parachains_inherent::ParachainsInherentDataProvider::create(
						&*client_clone,
						overseer_handle,
						parent,
...
> {
	...
	if config.chain_spec.is_cardamom() {
		return chain_ops!(config, jaeger_agent, telemetry_worker_handle; cardamom_runtime, CardamomExecutorDispatch, Cardamom)
	}

	#[cfg(feature = "elexeum-native")]
	{
		return chain_ops!(config, jaeger_agent, telemetry_worker_handle; elexeum_runtime, ElexeumExecutorDispatch, Elexeum)
	}
...
pub fn build_full(
	...
	#[cfg(feature = "cardamom-native")]
	if config.chain_spec.is_cardamom() {
		return new_full::<cardamom_runtime::RuntimeApi, CardamomExecutorDispatch, _>(
			config,
			is_collator,
			grandpa_pause,
			enable_beefy,
		...
		.map(|full| full.with_client(Client::Cardamom))
	}

	#[cfg(feature = "elexeum-native")]
	{
		return new_full::<elexeum_runtime::RuntimeApi, ElexeumExecutorDispatch, _>(
			config,
			is_collator,
			grandpa_pause,
			enable_beefy,
		...
		.map(|full| full.with_client(Client::Elexeum))
	}
