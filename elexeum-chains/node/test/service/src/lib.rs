...
use elexeum_node_primitives::{CollationGenerationConfig, CollatorFn};
use elexeum_node_subsystem::messages::{CollationGenerationMessage, CollatorProtocolMessage};
use elexeum_overseer::Handle;
use elexeum_primitives::v1::{Balance, CollatorPair, HeadData, Id as ParaId, ValidationCode};
use elexeum_runtime_common::BlockHashCount;
use elexeum_runtime_parachains::paras::ParaGenesisArgs;
use elexeum_service::{
	ClientHandle, Error, ExecuteWithClient, FullClient, IsCollator, NewFull, PrometheusConfig,
};
use elexeum_test_runtime::{
...
/// Declare an instance of the native executor named `ElexeumTestExecutorDispatch`. Include the wasm binary as the
/// equivalent wasm code.
pub struct ElexeumTestExecutorDispatch;

impl sc_executor::NativeExecutionDispatch for ElexeumTestExecutorDispatch {
	type ExtendHostFunctions = frame_benchmarking::benchmarking::HostFunctions;

	fn dispatch(method: &str, data: &[u8]) -> Option<Vec<u8>> {
		elexeum_test_runtime::api::dispatch(method, data)
	}

	fn native_version() -> sc_executor::NativeVersion {
		elexeum_test_runtime::native_version()
	}
}

/// The client type being used by the test service.
pub type Client = FullClient<elexeum_test_runtime::RuntimeApi, ElexeumTestExecutorDispatch>;

pub use elexeum_service::FullBackend;
...
pub fn new_full(
	...
	elexeum_service::new_full::<elexeum_test_runtime::RuntimeApi, ElexeumTestExecutorDispatch, _>(
		config,
		is_collator,
		None,
		true,
		...
		None,
		worker_program_path,
		elexeum_service::RealOverseerGen,
	)
}
...
impl ClientHandle for TestClient {
	fn execute_with<T: ExecuteWithClient>(&self, t: T) -> T::Output {
		T::execute_with_client::<_, _, elexeum_service::FullBackend>(t, self.0.clone())
	}
}
...
/// Create a Elexeum `Configuration`.
///
...
pub fn node_config(
	...
	let mut spec = elexeum_local_testnet_config();
	...
	Configuration {
		impl_name: "elexeum-test-node".to_string(),
...
pub fn run_validator_node(
	...
	let NewFull { task_manager, client, network, rpc_handlers, overseer_handle, .. } =
		new_full(config, IsCollator::No, worker_program_path)
			.expect("could not create Elexeum test service");

	let overseer_handle = overseer_handle.expect("test node must have an overseer handle");
	let peer_id = network.local_peer_id().clone();
	let addr = MultiaddrWithPeerId { multiaddr, peer_id };

	ElexeumTestNode { task_manager, client, overseer_handle, addr, rpc_handlers }
}
...
/// [`ElexeumTestNode::register_collator`].
...
/// A Elexeum test node instance used for testing.
pub struct ElexeumTestNode {
...
impl ElexeumTestNode {
	/// Send an extrinsic to this node.
	pub async fn send_extrinsic(
		&self,
		function: impl Into<elexeum_test_runtime::Call>,
...
pub fn construct_extrinsic(
	...
	UncheckedExtrinsic::new_signed(
		function.clone(),
		elexeum_test_runtime::Address::Id(caller.public().into()),
		elexeum_primitives::v0::Signature::Sr25519(signature.clone()),
		extra.clone(),
	)
}
...
pub fn construct_transfer_extrinsic(
	...
	let function = elexeum_test_runtime::Call::Balances(pallet_balances::Call::transfer {
