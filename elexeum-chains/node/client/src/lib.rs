...
#[cfg(not(any(feature = "elexeum", feature = "cardamom",)))]
compile_error!("at least one runtime feature must be enabled");

#[cfg(feature = "elexeum")]
/// The native executor instance for Elexeum.
pub struct ElexeumExecutorDispatch;

#[cfg(feature = "elexeum")]
impl sc_executor::NativeExecutionDispatch for ElexeumExecutorDispatch {
	type ExtendHostFunctions = frame_benchmarking::benchmarking::HostFunctions;

	fn dispatch(method: &str, data: &[u8]) -> Option<Vec<u8>> {
		elexeum_runtime::api::dispatch(method, data)
	}

	fn native_version() -> sc_executor::NativeVersion {
		elexeum_runtime::native_version()
	}
}

#[cfg(feature = "cardamom")]
/// The native executor instance for Cardamom.
pub struct CardamomExecutorDispatch;

#[cfg(feature = "cardamom")]
impl sc_executor::NativeExecutionDispatch for CardamomExecutorDispatch {
	type ExtendHostFunctions = frame_benchmarking::benchmarking::HostFunctions;

	fn dispatch(method: &str, data: &[u8]) -> Option<Vec<u8>> {
		cardamom_runtime::api::dispatch(method, data)
	}

	fn native_version() -> sc_executor::NativeVersion {
		cardamom_runtime::native_version()
	}
}

/// A set of APIs that elexeum-like runtimes must implement.
...
///
/// As there exist multiple chains inside Elexeum, like Elexeum itself, Cardamom etc,
...
/// A handle to a Elexeum client instance.
///
/// The Elexeum service supports multiple different runtimes (Cardamom, Elexeum itself, etc). As each runtime has a
...
macro_rules! with_client {
	{
		...
		match $self {
			#[cfg(feature = "cardamom")]
			Self::Cardamom($client) => { $( $code )* },
			#[cfg(feature = "elexeum")]
			Self::Elexeum($client) => { $( $code )* }
		}
	}
}

/// A client instance of Elexeum.
///
...
pub enum Client {
	...
	#[cfg(feature = "elexeum")]
	Elexeum(Arc<FullClient<elexeum_runtime::RuntimeApi, ElexeumExecutorDispatch>>),
}
