# Run a Validator on Elexeum-Node  
This guide will instruct you how to set up a validator node on the **Elexeum** network.

## How many ZARYN do I need ?​
 To became **Elexeum** validator you need to stake at least **31416 ZARYN**. This stake amount can come from yourself or from nominators. This means that as a minimum, you will need enough **ZARYN** to set up Stash and Controller accounts with the existential deposit, plus a little extra for transaction fees. The rest can come from nominators.
 
 ***Warning***: Any **ZARYN** that you stake for your validator is liable to be slashed, meaning that an insecure or improper setup may result in loss of **ZARYN** tokens! If you are not confident in your ability to run a validator node, it is recommended to nominate your SEL to a trusted validator node instead.
...
***WARNING***: Skipping this can result in the validator node missing block authorship opportunities. If the clock is out of sync (even by a small amount), the blocks the validator produces may not get accepted by the network. This will result in ImOnline heartbeats making it on chain, but zero allocated blocks making it on chain.
 ...
 ## Run Elexeum-Node from source
 To Install and build Elexeum-Node see [here](https://github.com/elexeum/elexeum-node/blob/main/docs/from_source.md).
 How run validator node see [here](https://github.com/elexeum/elexeum-node/blob/main/docs/run_validate_node.md).

 ## Run Elexeum-Node from docker

To run Elexeum-Node with docker see [here](https://github.com/elexeum/elexeum-node/blob/main/docs/from_docker.md).

## Bond ZARYN

It is highly recommended that you make your controller and stash accounts be two separate accounts. For this, you will create two accounts and make sure each of them have at least enough funds to pay the fees for making transactions. Keep most of your funds in the stash account since it is meant to be the custodian of your staking funds.
Make sure not to bond all your **ZARYN** balance since you will be unable to pay transaction fees from your bonded balance.
It is now time to set up our validator. We will do the following:

Bond the **ZARYN** of the Stash account. These **ZARYN** will be put at stake for the security of the network and can be slashed.
