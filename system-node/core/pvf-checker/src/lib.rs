	...
	Context: overseer::SubsystemContext<Message = PvfCheckerMessage>,
{
	fn start(self, ctx: Context) -> SpawnedSubsystem {
		if self.enabled {
			...
			elexeum_overseer::DummySubsystem.start(ctx)
		}
	}
}
...
async fn check_signing_credentials(
	...
	elexeum_node_subsystem_util::signing_key_and_index(&validators, keystore)
		.await
...
async fn sign_and_submit_pvf_check_statement(
	...
	let signature = match elexeum_node_subsystem_util::sign(
		keystore,
