...
use bitvec::bitvec;
use elexeum_primitives::v1::{OccupiedCore, ScheduledCore};  
...
mod select_availability_bitfields {
	...
	use elexeum_primitives::v1::{ScheduledCore, SigningContext, ValidatorId, ValidatorIndex};
...
mod select_candidates {
	...
	use ::test_helpers::{dummy_candidate_descriptor, dummy_hash};
	use elexeum_node_subsystem::messages::{
	...
	fn test_harness<OverseerFactory, Overseer, TestFactory, Test>(
		...
		Test: Future<Output = ()>,
	{
		let (tx, rx) = elexeum_node_subsystem_test_helpers::sender_receiver();
