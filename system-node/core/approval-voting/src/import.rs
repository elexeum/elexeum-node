...
use elexeum_node_jaeger as jaeger;
use elexeum_node_primitives::approval::{
	self as approval_types, BlockApprovalMeta, RelayVRFStory,
};
use elexeum_node_subsystem::{
	messages::{
...
use elexeum_node_subsystem_util::{
...
async fn imported_block_info(
	...
	let babe_epoch = {
		...
		//
		// It's worth nothing that elexeum session changes, at least for the purposes of parachains,
...
pub(crate) mod tests {
	...
	use elexeum_node_primitives::approval::{VRFOutput, VRFProof};
	use elexeum_node_subsystem::messages::AllMessages;
	use elexeum_node_subsystem_test_helpers::make_subsystem_context;
	use elexeum_primitives::{v1::ValidatorIndex, v2::SessionInfo};
	...
	impl AssignmentCriteria for MockAssignmentCriteria {
		fn compute_assignments(
			...
			_relay_vrf_story: elexeum_node_primitives::approval::RelayVRFStory,
			_config: &criteria::Config,
			_leaving_cores: Vec<(
				CandidateHash,
				elexeum_primitives::v1::CoreIndex,
				elexeum_primitives::v1::GroupIndex,
			)>,
		) -> HashMap<elexeum_primitives::v1::CoreIndex, criteria::OurAssignment> {
			HashMap::new()
		}

		fn check_assignment_cert(
			&self,
			_claimed_core_index: elexeum_primitives::v1::CoreIndex,
			_validator_index: elexeum_primitives::v1::ValidatorIndex,
			_config: &criteria::Config,
			_relay_vrf_story: elexeum_node_primitives::approval::RelayVRFStory,
			_assignment: &elexeum_node_primitives::approval::AssignmentCert,
			_backing_group: elexeum_primitives::v1::GroupIndex,
		) -> Result<elexeum_node_primitives::approval::DelayTranche, criteria::InvalidAssignment> {
			Ok(0)
		}
	}
