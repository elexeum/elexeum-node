...
use elexeum_parachain::primitives::{ValidationParams, ValidationResult as WasmValidationResult};
...
pub struct CandidateValidationSubsystem {
	...
	pub metrics: Metrics,
	#[allow(missing_docs)]
	pub pvf_metrics: elexeum_node_core_pvf::Metrics,
	config: Config,
}
...
impl CandidateValidationSubsystem {
	...
	pub fn with_config(
		config: Config,
		metrics: Metrics,
		pvf_metrics: elexeum_node_core_pvf::Metrics,
	) -> Self {
...
where
	...
	Context: overseer::SubsystemContext<Message = CandidateValidationMessage>,
{
	let (validation_host, task) = elexeum_node_core_pvf::start(
		elexeum_node_core_pvf::Config::new(cache_path, program_path),
		pvf_metrics,
	);
	...
	async fn validate_candidate(
		...
		if let Err(err) = self
			.execute_pvf(
				...
				timeout,
				params.encode(),
				elexeum_node_core_pvf::Priority::Normal,
				tx,
			)
			.await
		{
		...
		let metrics = MetricsInner {
			validation_requests: prometheus::register(
				prometheus::CounterVec::new(
					prometheus::Opts::new(
						"elexeum_parachain_validation_requests_total",
			...
			validate_from_chain_state: prometheus::register(
				prometheus::Histogram::with_opts(prometheus::HistogramOpts::new(
					"elexeum_parachain_candidate_validation_validate_from_chain_state",
			...
			validate_from_exhaustive: prometheus::register(
				prometheus::Histogram::with_opts(prometheus::HistogramOpts::new(
					"elexeum_parachain_candidate_validation_validate_from_exhaustive",
			...
			validate_candidate_exhaustive: prometheus::register(
				prometheus::Histogram::with_opts(prometheus::HistogramOpts::new(
					"elexeum_parachain_candidate_validation_validate_candidate_exhaustive",
