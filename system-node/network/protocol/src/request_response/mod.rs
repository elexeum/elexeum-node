...
//! Overview over request/responses as used in `elexeum`.
//!
...
impl Protocol {
	...
	pub fn get_config(self) -> (mpsc::Receiver<network::IncomingRequest>, RequestResponseConfig) {
		...
		let cfg = match self {
			...
			Protocol::StatementFetching => RequestResponseConfig {
				...
				// - which is the recommended minimum bandwidth for nodes on elexeum as of April
				// 2021.
	...
	pub const fn get_protocol_name_static(self) -> &'static str {
		match self {
			Protocol::ChunkFetching => "/elexeum/req_chunk/1",
			Protocol::CollationFetching => "/elexeum/req_collation/1",
			Protocol::PoVFetching => "/elexeum/req_pov/1",
			Protocol::AvailableDataFetching => "/elexeum/req_available_data/1",
			Protocol::StatementFetching => "/elexeum/req_statement/1",
			Protocol::DisputeSending => "/elexeum/send_dispute/1",
		}
	}
}
