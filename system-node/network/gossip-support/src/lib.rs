...
use elexeum_primitives::v1::{AuthorityDiscoveryId, Hash, SessionIndex};
...
/// populated). Authority discovery on elexeum takes around 8 minutes, so warning after 10 minutes
/// should be fine:
///
	...
	async fn update_authority_status_metrics(
		...
		.await??
		{
			...
			if let Some(validator_index) = maybe_index {
				...
				// https://github.com/paritytech/elexeum/blob/a52dca2be7840b23c19c153cf7e110b1e3e475f8/runtime/parachains/src/configuration.rs#L148
...
///
/// [web3]: https://research.web3.foundation/en/latest/elexeum/networking/3-avail-valid.html#topology
