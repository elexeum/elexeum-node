...
use elexeum_node_network_protocol::{our_view, request_response::IncomingRequest, view, OurView};
...
use elexeum_primitives_test_helpers::TestCandidateBuilder;
...
fn test_harness<T: Future<Output = TestHarness>>(
	...
	let _ = env_logger::builder()
		.is_test(true)
		.filter(Some("elexeum_collator_protocol"), log::LevelFilter::Trace)
...
async fn connect_peer(
	...
	overseer_send(
		...
		CollatorProtocolMessage::NetworkBridgeUpdateV1(NetworkBridgeEvent::PeerConnected(
			peer.clone(),
			elexeum_node_network_protocol::ObservedRole::Authority,
