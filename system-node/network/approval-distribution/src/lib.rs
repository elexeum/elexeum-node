		...
		if let Some(peer_id) = source.peer_id() {
			...
			match result {
				...
				AssignmentCheckResult::AcceptedDuplicate => {
					...
					// cf. https://github.com/paritytech/elexeum/pull/2160#discussion_r557628699
					if let Some(peer_knowledge) = entry.known_by.get_mut(&peer_id) {
						peer_knowledge.received.insert(fingerprint);
					}
					tracing::debug!(
		...
		let metrics = MetricsInner {
			assignments_imported_total: prometheus::register(
				prometheus::Counter::new(
					"elexeum_parachain_assignments_imported_total",
			...
			approvals_imported_total: prometheus::register(
				prometheus::Counter::new(
					"elexeum_parachain_approvals_imported_total",
			...
			unified_with_peer_total: prometheus::register(
				prometheus::Counter::new(
					"elexeum_parachain_unified_with_peer_total",
			...
			time_unify_with_peer: prometheus::register(
				prometheus::Histogram::with_opts(prometheus::HistogramOpts::new(
					"elexeum_parachain_time_unify_with_peer",
			...
			time_import_pending_now_known: prometheus::register(
				prometheus::Histogram::with_opts(prometheus::HistogramOpts::new(
					"elexeum_parachain_time_import_pending_now_known",
			...
			time_awaiting_approval_voting: prometheus::register(
				prometheus::Histogram::with_opts(prometheus::HistogramOpts::new(
					"elexeum_parachain_time_awaiting_approval_voting",
