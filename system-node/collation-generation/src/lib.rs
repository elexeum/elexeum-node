...
//! The collation generation subsystem is the interface between elexeum and the collators.

#![deny(missing_docs)]

use futures::{channel::mpsc, future::FutureExt, join, select, sink::SinkExt, stream::StreamExt};
use parity_scale_codec::Encode;
use elexeum_node_primitives::{AvailableData, CollationGenerationConfig, PoV};
		...
		for (core_idx, core) in availability_cores.into_iter().enumerate() {
			...
			ctx.spawn(
				...
				Box::pin(async move {
					...
					let pov = {
						let pov = elexeum_node_primitives::maybe_compress_pov(
...
async fn obtain_current_validation_code_hash(
	...
	use elexeum_node_subsystem::RuntimeApiError;
...
fn erasure_root(
	...
	let chunks = elexeum_erasure_coding::obtain_chunks_v1(n_validators, &available_data)?;
	Ok(elexeum_erasure_coding::branches(&chunks).root())
}
		...
		let metrics = MetricsInner {
			collations_generated_total: prometheus::register(
				prometheus::Counter::new(
					"elexeum_parachain_collations_generated_total",
			...
			new_activations_overall: prometheus::register(
				prometheus::Histogram::with_opts(
					prometheus::HistogramOpts::new(
						"elexeum_parachain_collation_generation_new_activations",
			...
			new_activations_per_relay_parent: prometheus::register(
				prometheus::Histogram::with_opts(
					prometheus::HistogramOpts::new(
						"elexeum_parachain_collation_generation_per_relay_parent",
			...
			new_activations_per_availability_core: prometheus::register(
				prometheus::Histogram::with_opts(
					prometheus::HistogramOpts::new(
						"elexeum_parachain_collation_generation_per_availability_core",
