...
pub use jaeger::*;
pub use elexeum_node_jaeger as jaeger;

pub use elexeum_overseer::{
	self as overseer, ActiveLeavesUpdate, OverseerConnector, OverseerSignal,
};

pub use elexeum_node_subsystem_types::{
	errors::{self, *},
...
pub mod messages {
	...
	pub use elexeum_node_subsystem_types::messages::*;
}
...
pub type FromOverseer<M> = elexeum_overseer::gen::FromOverseer<M, OverseerSignal>;

/// Specialized subsystem instance type of subsystems consuming a particular message type.
pub type SubsystemInstance<Message> =
	elexeum_overseer::gen::SubsystemInstance<Message, OverseerSignal>;

/// Sender trait for the `AllMessages` wrapper.
pub trait SubsystemSender: elexeum_overseer::gen::SubsystemSender<messages::AllMessages> {}

impl<T> SubsystemSender for T where T: elexeum_overseer::gen::SubsystemSender<messages::AllMessages>
{}

/// Spawned subsystem.
pub type SpawnedSubsystem = elexeum_overseer::gen::SpawnedSubsystem<SubsystemError>;

/// Convenience trait specialization.
pub trait SubsystemContext:
	elexeum_overseer::gen::SubsystemContext<
...
where
	T: elexeum_overseer::gen::SubsystemContext<
		Signal = OverseerSignal,
		AllMessages = messages::AllMessages,
		Error = SubsystemError,
	>,
{
	type Message = <Self as elexeum_overseer::gen::SubsystemContext>::Message;
	type Sender = <Self as elexeum_overseer::gen::SubsystemContext>::Sender;
}
