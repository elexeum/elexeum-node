...
pub use elexeum_core_primitives::v1::{
...
// Export some elexeum-parachain primitives
pub use elexeum_parachain::primitives::{
...
pub mod well_known_keys {
	...
	///
	/// The storage entry stores a `Hash`. This is elexeum hash which is at the moment
	/// `blake2b-256`.
...
pub struct SessionInfo {
	...
	/// [`max_validators`](https://github.com/paritytech/elexeum/blob/a52dca2be7840b23c19c153cf7e110b1e3e475f8/runtime/parachains/src/configuration.rs#L148).
	///
...
/// Custom validity errors used in Elexeum while validating transactions.
#[repr(u8)]
...
/// Consensus engine id for elexeum v1 consensus engine.
pub const ELEXEUM_ENGINE_ID: runtime_primitives::ConsensusEngineId = *b"POL1";

/// A consensus log item for elexeum validation. To be used with [`ELEXEUM_ENGINE_ID`].
...
impl ConsensusLog {
	...
	pub fn from_digest_item(
		...
		match digest_item {
			runtime_primitives::DigestItem::Consensus(id, encoded) if id == &ELEXEUM_ENGINE_ID =>
...
impl From<ConsensusLog> for runtime_primitives::DigestItem {
	fn from(c: ConsensusLog) -> runtime_primitives::DigestItem {
		Self::Consensus(ELEXEUM_ENGINE_ID, c.encode())
	}
}
