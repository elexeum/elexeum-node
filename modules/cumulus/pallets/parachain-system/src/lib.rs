...
use elexeum_parachain::primitives::RelayChainBlockNumber;
use sp_runtime::{
...
pub mod pallet {
	...
	pub enum Error<T> {
		...
		OverlappingUpgrades,
		/// Elexeum currently prohibits this parachain from upgrading its validation function
		ProhibitedByElexeum,
		/// The supplied validation function has compiled into a blob larger than Elexeum is
		/// willing to run
		TooBig,
...
impl<T: Config> Pallet<T> {
	...
	/// [`ValidationParams`](elexeum_parachain::primitives::ValidationParams).
	///
	...
	/// Put a new validation function into a particular location where elexeum
	/// monitors for updates. Calling this function notifies elexeum that a new
	/// upgrade has been scheduled.
	fn notify_elexeum_of_pending_upgrade(code: &[u8]) {
	...
	pub fn schedule_code_upgrade(validation_function: Vec<u8>) -> DispatchResult {
		...
		ensure!(<UpgradeRestrictionSignal<T>>::get().is_none(), Error::<T>::ProhibitedByElexeum);
		...
		// places, synchronized: both elexeum and the individual parachain
		// have to upgrade on the same relay chain block.
		//
		// `notify_elexeum_of_pending_upgrade` notifies elexeum; the `PendingValidationCode`
		...
		Self::notify_elexeum_of_pending_upgrade(&validation_function);
