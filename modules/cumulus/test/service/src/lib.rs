...
use elexeum_primitives::v1::{CollatorPair, Hash as PHash, PersistedValidationData};
use elexeum_service::ProvideRuntimeApi;
		...
		+ 'static,
{
	...
	let relay_chain_full_node = elexeum_test_service::new_full(
		relay_chain_config,
		if let Some(ref key) = collator_key {
			elexeum_service::IsCollator::Yes(key.clone())
		} else {
			elexeum_service::IsCollator::Yes(CollatorPair::generate().0)
		},
		None,
	)
	.map_err(|e| match e {
...
impl TestNodeBuilder {
	...
	pub fn connect_to_relay_chain_node(
		mut self,
		node: &elexeum_test_service::ElexeumTestNode,
	) -> Self {
	...
	pub fn connect_to_relay_chain_nodes<'a>(
		mut self,
		nodes: impl IntoIterator<Item = &'a elexeum_test_service::ElexeumTestNode>,
	) -> Self {
	...
	pub async fn build(self) -> TestNode {
		...
		let mut relay_chain_config = elexeum_test_service::node_config(
...
/// [`run_validator_node`](elexeum_test_service::run_validator_node).
pub fn run_relay_chain_validator_node(
	tokio_handle: tokio::runtime::Handle,
	key: Sr25519Keyring,
	storage_update_func: impl Fn(),
	boot_nodes: Vec<MultiaddrWithPeerId>,
) -> elexeum_test_service::ElexeumTestNode {
	let config = elexeum_test_service::node_config(
		storage_update_func,
		tokio_handle,
		key,
		boot_nodes,
	...
	elexeum_test_service::run_validator_node(
		config,
