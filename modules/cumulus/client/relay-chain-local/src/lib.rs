...
use elexeum_client::{ClientHandle, ExecuteWithClient, FullBackend};
use elexeum_service::{
...
///
/// The builder takes a [`elexeum_client::Client`]
/// that wraps a concrete instance. By using [`elexeum_client::ExecuteWithClient`]
...
struct RelayChainLocalBuilder {
	elexeum_client: elexeum_client::Client,
	backend: Arc<FullBackend>,
	sync_oracle: Arc<Mutex<Box<dyn SyncOracle + Send + Sync>>>,
	overseer_handle: Option<Handle>,
}
...
impl RelayChainLocalBuilder {
	pub fn build(self) -> Arc<dyn RelayChainInterface> {
		self.elexeum_client.clone().execute_with(self)
	}
}
...
/// Build the elexeum full node using the given `config`.
#[sc_tracing::logging::prefix_logs_with("Relaychain")]
fn build_elexeum_full_node(
	config: Configuration,
	telemetry_worker_handle: Option<TelemetryWorkerHandle>,
) -> Result<(NewFull<elexeum_client::Client>, CollatorPair), elexeum_service::Error> {
	let is_light = matches!(config.role, Role::Light);
	if is_light {
		Err(elexeum_service::Error::Sub("Light client not supported.".into()))
	} else {
		let collator_key = CollatorPair::generate().0;

		let relay_chain_full_node = elexeum_service::build_full(
			config,
			elexeum_service::IsCollator::Yes(collator_key.clone()),
			None,
			true,
			None,
...
pub fn build_relay_chain_interface(
	elexeum_config: Configuration,
	telemetry_worker_handle: Option<TelemetryWorkerHandle>,
	task_manager: &mut TaskManager,
) -> Result<(Arc<(dyn RelayChainInterface + 'static)>, CollatorPair), elexeum_service::Error> {
	let (full_node, collator_key) =
		build_elexeum_full_node(elexeum_config, telemetry_worker_handle).map_err(
			|e| match e {
				elexeum_service::Error::Sub(x) => x,
				s => format!("{}", s).into(),
			},
		)?;
	...
	let relay_chain_interface_builder = RelayChainLocalBuilder {
		elexeum_client: full_node.client.clone(),
...
mod tests {
	...
	use sc_service::Arc;
	use elexeum_primitives::v1::Block as PBlock;
	use elexeum_test_client::{
		construct_transfer_extrinsic, BlockBuilderExt, Client, ClientBlockImportExt,
		DefaultTestClientBuilderExt, ExecutionStrategy, InitElexeumBlockBuilder,
	...
	fn build_client_backend_and_block() -> (Arc<Client>, PBlock, RelayChainLocal<Client>) {
		...
		let block_builder = client.init_elexeum_block_builder();
	...
	fn do_not_resolve_after_different_block_import_notification_was_received() {
		...
		let mut block_builder = client.init_elexeum_block_builder();
		// Push an extrinsic to get a different block hash.
		block_builder.push_elexeum_extrinsic(ext).expect("Push extrinsic");
