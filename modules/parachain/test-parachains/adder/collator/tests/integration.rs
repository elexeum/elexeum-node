...
async fn collating_using_adder_collator() {
	use elexeum_primitives::v1::Id as ParaId;
	...
	let alice_config = elexeum_test_service::node_config(
		|| {},
	...
	// start alice
	let alice = elexeum_test_service::run_validator_node(alice_config, Some(PUPPET_EXE.into()));

	let bob_config = elexeum_test_service::node_config(
		|| {},
		tokio::runtime::Handle::current(),
		Bob,
	...
	// start bob
	let bob = elexeum_test_service::run_validator_node(bob_config, Some(PUPPET_EXE.into()));
	...
	let mut charlie = elexeum_test_service::run_collator_node(
		tokio::runtime::Handle::current(),
		Charlie,
		|| {},
