...
use mock::{
	elexeum_like_with_balances, AccountId, Balance, Balances, BaseXcmWeight, XcmConfig, CENTS,
};
...
fn withdraw_and_deposit_works() {
	...
	elexeum_like_with_balances(balances).execute_with(|| {
...
fn teleport_to_indracore_works() {
	...
	elexeum_like_with_balances(balances).execute_with(|| {
		...
		// teleports are allowed from indracore to elexeum.
		let r = XcmExecutor::<XcmConfig>::execute_xcm(
			Parachain(PARA_ID).into(),
			Xcm(vec![
				WithdrawAsset((Here, amount).into()),
