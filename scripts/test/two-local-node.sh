...
# chainspec defaults to elexeum-local if no arguments are passed to this script;
...
elexeum="target/release/elexeum"

# ensure the elexeum binary exists and is up to date
if [ ! -x "$elexeum" ] || [ "$elexeum" -ot "$last_modified_rust_file" ]; then
  cargo build --release
fi
...
# The name must be one of those which can be passed to the elexeum binary, in un-flagged form,
# one of:
